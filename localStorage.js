...
/**
 * Set the grid of the local storage with the key and size of local storage entries
 *
 * @param {object} grid dijit grid
 */
function localStorageSetStore(grid) {
	...
	for (var z = 0; z < localStorage.length; z++) {
		...
		localItems.push({index: count, size: localStorage.getItem(key).length, key: key, query: localStorage.getItem(key)});
		count++;
...
/***
 *
 * @param {type} entryKey key to be shown
 * @returns {undefined} null
 */
