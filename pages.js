...
function getRpage() {
    ...
    if (globalPageMangler)
    {
        ...
        throw('No page mangler found');
    }
}
function getQuerypage() {
    var pageTypes = ['query.php', 'query.jsp'];
    var page;
    if (globalPageQuery)
    {
        return globalPageQuery;
    }
    else {
        for (p in pageTypes) {
            page = pageTypes[p];
            try {
                xmlhttp = new XMLHttpRequest();
                xmlhttp.open('HEAD', page + '?method=list', false);
                xmlhttp.send();
                globalPageQuery = page;
                return page;
            } catch (e) {
                console.log('Query page {0} not there:{1}'.format(page, e.message));
            }
        }
        throw('No query page found'.format());
    }
}
