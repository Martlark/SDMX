...
function _renderData(data) {
	...
	for (var z in data) {
		...
		for (c in countryCodeList) {
			if (countryCodeList[c].code == data[z][globalOptions.breakField]) {
	...
	for (z in data) {
		...
		//console.log( data[z].Country, data[z][globalOptions.breakField], data[z].Value );
	}
	return renderer;
}

function _dialogCompareChart(idLocation, currentIndex, data) {
	// adds a chart to the mouse over dialog of the highest and lowest globalOptions.breakField values
	...
	labels.push(data[highest][globalOptions.breakField]);
	if (currentIndex != highest && currentIndex != lowest) {
		// don't include current twice
		series.push(data[currentIndex].Value);
		labels.push(data[currentIndex][globalOptions.breakField]);
	}
	if (highest != lowest) {
		series.push(data[lowest].Value);
		labels.push(data[lowest][globalOptions.breakField]);
	}
...
//	if( !evt.graphic.attributes.DISPLAY_VALUE ){
	...
	for (z in currentData) {
		if (currentData[z].Country == countryName) {
			...
			evt.graphic.attributes[globalOptions.breakField] = currentData[z][globalOptions.breakField];
			currentIndex = z;
			break;
		}
	}
...
function _onClick(evt) {
	...
	for (var z in currentData) {
		if (currentData[z].Country == currentArea) {
			var seriesKey = currentData[z][globalOptions.breakField];
//			console.log( seriesKey );
			// add or remove the series for this country
			mapChartResultsAddSeries(sdmxData.data, globalOptions.breakField, seriesKey, currentArea);
		}
	}
}
...
function mapResultResponse(xmlhttp, query) {
	...
	if (xmlhttp.responseText) {
		sdmxData = parseQueryDataResponse(xmlhttp, globalOptions.breakField);
	...
	function _addPopulationToData(countryPopulationList, data) {
		...
		for (var d in data) {
			var area = data[d][globalOptions.breakField];
